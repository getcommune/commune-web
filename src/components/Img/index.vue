  ...
  setup(_props, { attrs, slots, emit }) {
    ...
    const heightAndWidth = {
      width: props.value.width
        ? `${parseFloat(String(props.value.width))}px`
        : undefined,
      height: props.value.height
        ? `${parseFloat(String(props.value.height))}px`
        : undefined,
    };
    ...
    const wrapper = (node: any) =>
      [
        ...
        intersected.value ? node : anchorImage,
      ...
      if (props.value.loadEffect) {
        ...
        return {
          ...
          "--filter":
            loadEffect === "blur"
              ? loading.value
                ? `blur(${Number(props.value.blurRadius)}px)`
                : undefined
              : undefined,
          "--transition": `${
            loadEffect === "fade" ? "opacity" : "filter"
          } ${convertToMilliSecond(
            loadEffectDuration
          )}ms ${loadEffectEase} ${convertToMilliSecond(loadEffectDelay)}ms`,
        };
      }
      return {};
    });
